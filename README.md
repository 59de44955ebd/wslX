...
* Implementing drag-and-drop also for the opposite direction - from WSL Linux file managers into arbitrary Windows application windows - would be tricky, it would require to inject a custom .dll into every application process for hooking its window procedure. But wslX at least automatically converts filesystem content in the Linux/X clipboard to CF_HDROP content in the Windows clipboard (again automatically converting paths for the target system), so you can copy files and folders in a WSL Linux file manager and then paste them e.g. into a Windows explorer window to copy them. This was so far successfully tested with Xfce's default file manager Thunar, PCManFM, Dolphin and Nautilus.
